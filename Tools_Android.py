...
class TauxAlc():
    ...
    def taux0(self, username, aff=True):
        ...
        if aff:
            if horZero > time.mktime(time.strptime(str(time.strftime("%b %d %H:%M %Y", time.localtime())),"%b %d %H:%M %Y")):
                tmp=str(datetime.fromtimestamp(horZero))[:-3]
                if tmp[:-6]==str(datetime.now())[:-16]:
                    tmp= f"aujourd'hui à {tmp[11:]}"
                    phr="Sobre "+ tmp
                elif tmp[:-6]==str(datetime.now()+timedelta(1))[:-16]:
                    tmp= f"demain à {tmp[11:]}"
                    phr="Sobre "+ tmp
                else:
                    phr= "Sobre le: "+ tmp
    ...
    def tauxcond(self, username):
        ...
        if horCond[:-6]==str(datetime.now())[:-16]:
            horCond= f"aujourd'hui à {horCond[11:]}"
            phr="Vous pourrez conduire "+ horCond
        elif horCond[:-6]==str(datetime.now()+timedelta(1))[:-16]:
            horCond= f"demain à {horCond[11:]}"
            phr="Vous pourrez conduire "+ horCond
        else:
            phr= "Vous pourrez conduire le: "+ horCond
        return phr
    ...
    def conduiteaut(self, username):
        ...
        if records==[]:
            ...
            if horaire<horMaxi and taux <= self.limit:
                phr = f"Attention, pic d'alcoolémie à {str(datetime.fromtimestamp(horMaxi))[11:-3]}!"
                return phr
